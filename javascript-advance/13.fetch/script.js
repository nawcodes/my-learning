...
const searchButton = document.querySelector('.btn-search');
    // mau apa dengan tombol tsb
searchButton.addEventListener('click', function () {
  
    // ambil input keyword
    const inputKeyword = document.querySelector('.input-keyword');

    fetch('http://www.omdbapi.com/?apikey=93670083&s=' + inputKeyword.value)
        ...
        .then(response => {
            ...
            // detail 
            const modalDetailButton = document.querySelectorAll('.btn-modal-show'); //result nodelist

            modalDetailButton.forEach(btn => {
                 btn.addEventListener('click' , function(){
                    const imdbid = this.dataset.imdbid;
                    fetch('http://www.omdbapi.com/?apikey=93670083&i=' + imdbid)
                    .then(response => response.json())
                    .then(m => {
